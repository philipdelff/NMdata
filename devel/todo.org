...
*** NMgenText
**** TODO [#A] Add a last/stop argument to specify the last column to use in nonmem
...
*** mergeCheck
**** TODO accept common columns not being merged by if suffixes are provided
**** TODO If results are not accepted. Check for whether y is unique in by
...
*** NMextractDataFile
**** TODO [#A] Test with absolute path in $DATA
...
*** NMscanData
**** TODO [#B] Split out data finder
There is a few different ways to get to the data file path. A separate
function should handle this. That same function can then be used by
NMcheckData.

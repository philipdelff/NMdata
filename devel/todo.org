...
*** TODO NMsummarizeData Summarize data with nonmem in mind. 
...
*** compareCols
**** TODO [#B] Bug - overview of columns is given twice
 compareCols(ex.001,ex.002,ex.005,diff.only=T) %>% print(topn=100)
**** TODO [#B] scope argument
...
**** TODO [#A] ID and row identifier must not start with a zero
     That would ruin merging if Nonmem prints as a double or integer
**** TODO [#A] Should more of the checked columns be checked for NMisNumeric?
**** DONE [#A] check RATE, SS
**** TODO [#B] Check ADDL, II
**** TODO [#A] Support for file argument (control stream)
**** DONE Element-wise NMisNumeric
     CLOSED: [2021-12-16 Thu 21:47]
...
*** NMwriteData
**** TODO [#A] Remove check for standard columns
     This is done in NMcheckData
**** TODO [#B] Separate generation of INPUT text into new function
...
*** Cheat sheet
**** TODO Flag example
*** TODO [#A] input.txt/output.txt example must be generated with plain nonmem
    Test NMdataConf(file.mod=identity)
*** DONE renameByContents example
    CLOSED: [2021-12-19 Sun 20:45]
** Discussion

...
*** addTAPD
**** TODO [#A] Not evaluated at dose times for other doses than captured by subset
...
*** NMwriteSection
**** TODO [#B] Support insertion of new section before or after another section
location="replace" is default. "before" and "after" should be
additional allowed values. Think about how to handle this for
...
*** NMscanInput
**** TODO Bug: Error when ID not in data but var=ID in $INPUT

...
*** addTAPD
**** TODO [#A] Rename NDOSES to DOSCUMN
It aligns with DOSCUMA and it is clearer from DOSCUMN that its
cumulative number of doses. NDOSES could be total number of doses.
**** DONE Bug in DOSCUMA
...
**** TODO [#A] It should not be a finding if col.flagn is not supplied and FLAG is not found
**** TODO [#A] DV=0 in dosing records is OK
For now NA expected

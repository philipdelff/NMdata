...
*** NMwriteSection
**** TODO [#A] Allow to remove a section if it doesn't exist
...
list.
*** NMexpandDoses
**** TODO [#A] col.time must be in data
**** TODO [#A] col.id must be in data
**** TODO [#A] Return data.table when working with data.table?
...
*** NMscanInput
**** TODO [#A] Test with multiple filters on the same column
...
*** NMwriteData
**** TODO [#A] Support not writing all columns to csv

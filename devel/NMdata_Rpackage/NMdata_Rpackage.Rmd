...
Nonmem almost entirely based) on numeric data values.  
* The source data
will often contain character variables, i.e. columns with non-numeric
data values.
* If the column names reflect whether the values are numeric, double-checking can be avoided. 
...
```{r}
nmCode <- NMwriteData(pk,file="derived/pk.csv",write.csv=FALSE,
                      nmdir.data="../derived",nmdrop="PROFDAY",nm.rename=c(CONC="DV"))
...
`NMextractDataFile` takes a control stream/list file and extracts the input data file name/path. You can use this to identify the model runs to update.
...
:::: column
```{r}
file1.lst <- system.file("examples/nonmem/xgxr001.lst",
                         package="NMdata")
...
::: {.column width="55%"}
\normalsize
```{r,compmodels,eval=T,echo=FALSE,out.width="105%"}
```
...
* The next milestone is submitting the package to CRAN
  - Little left on the todo list before submission

...
the existing datasets. Say we want to add a covariate from a `dt.cov`. Imagine we just assume that all subjects are represented with one row each in `dt.cov`, and so we expect the number of rows to be unchanged from `pk`:
...
<!-- The implementation in `NMdata` is based on sequentially checking conditions th -->

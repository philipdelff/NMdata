...
With the flexibility of the `rds` format, we don't need such an additional file:
```{r}
...
We saw how `NMwriteDatat` saves metadata automatically. Even if we `NMwriteData` can actually be used as a simple rds writer that adds meta data the same way, we may want to save data for analysis in R using `writeRDS`. In that case, use `NMstamp` (which is also what `NMwriteData` does).

...
## Adding time since previous dose, cumulative number of doses etc.
...
pk <- addTAPD(pk)
cnames.2 <- colnames(pk)
setdiff(cnames.2,cnames.1)
```

By default, the column names shown above are used. `addTAPD` takes
arguments to customize the generated column names and of course to indicate what columns are store the used information, such as dose amounts, time etc

---
title: "NMscanData: Scan for and combine all output and input data"
output:
...
vignette: >
  %\VignetteIndexEntry{NMscanData: Scan for and combine all output and input data}
...
<!-- Use NMscanData to  -->
<!-- Read and combine all output and input data based only on (the path to) the Nonmem list file -->

<!-- Include input data rows that were not processed by Nonmem (`ACCEPT` and `IGNORE`) -->

<!-- Combine such data sets for of a multiple models -->
...
data. After scanning the Nonmem list file and/or control stream for file and column names, the data files are read and combined. 

In brevity, the most important steps are

- Based on .lst/.mod: Identify input and output table files 
...
output and input data. Also, da

The following main
principles are followed by `NMscanData`:

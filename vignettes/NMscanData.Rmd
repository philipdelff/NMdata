...
## Get started
Just one thing before we try this out. `NMscanData` and other data
reader functions in `NMdata` return objects of class `data.table` by
default. We will start by changing this behaviour so we will get
a `data.frame` back instead. If you prefer `data.table`, skip this
step. If you prefer the `tibble` class (for dplyr and rest of
tidyverse), load the `tibble` package and insert `tibble::as_tibble`. 

```{r}
options(NMdata.as.fun=as.data.frame)
```

Now try NMscanData on any model:
```{r,eval=TRUE}
...
because it does not rely on assumptions about the input data and hence
very generic. It
happens that this dataset and one of the output tables contain a row
identifier (a row counter in the column called `ROW`). If we skipped
the `cbind.by.filters` above, `NMscanData` would figure this out and
recommend merging by this column. This is what we will now do using
the `col.row` argument. The following

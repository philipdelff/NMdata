...
`mergeCheck(x1,x2,...)` - Merges data and only accept results if all that happened was that columns from `x2` were added to `x1`. Row order of `x1` is retained. Arguments are passed to data.table which does the actual merge. This completely automates the necessary checks when say merging covariates onto data.

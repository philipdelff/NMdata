...
`NMdata 0.0.7` release, do (notice "@v":)
...
This may happen when you work with data.tables - and only when working
with data.tables. This will be fixed in near future. For now, you may
...
convention can easily be configured using the `file.mod`
option. If you input control streams are ending with `.cfg`, you can do

```{r,eval=FALSE}
NMdataConfig(file.mod=function(file) fnExtension(file,".cfg"))
```
where `fnExtension` is a very simple function in `NMdata` which covers this exact purpose. Feel free to use any other tools to put together a function that translates from list file name to input control stream file name.
...
located next to the list file called `output.txt`, you can use

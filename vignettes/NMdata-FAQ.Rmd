...
PSN (which is that the output control stream is called .lst and the
input control stream is located next to it called .mod).  This naming
...
If the name of the directory is the model name, you can do the
following as well:

```{r,eval=FALSE}
options(NMdata.modelname=function(file) basename(dirname(normalizePath(file))))
```
This will take the model name (used in `model` column in the returned
data and in summaries) from the name of the directory.
